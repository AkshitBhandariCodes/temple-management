...
import { Edit, Save, X, Upload, Plus, Crown, Shield, Trash2 } from "lucide-react";
...
export const CommunityOverview = ({ community, onUpdate }: CommunityOverviewProps) => {
  ...
  const { data: leads, isLoading: leadsLoading } = useCommunityLeads(community.id);
          ...
          <div className="flex items-center justify-between">
            ...
            {!isEditing ? (
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleSave} disabled={updateCommunity.isPending}>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setEditData(prev => ({ ...prev, publicVisibility: checked }))
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setEditData(prev => ({ ...prev, allowJoinRequests: checked }))
                    }
                  />
                </div>
              </CardContent>
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-medium">{community.owner?.name || 'Unknown'}</p>
                  <p className="text-sm text-muted-foreground">{community.owner?.email}</p>
                </div>
              </div>
              <Select onValueChange={(memberId) => {
                const member = members?.find((m: any) => m._id === memberId);
                if (member) handleChangeOwner(memberId, member.user_id._id);
              }}>
                ...
                <SelectContent>
                  {availableMembers.map((member: any) => (
                    <SelectItem key={member._id} value={member._id}>
                      {member.user_id?.full_name || 'Unknown'}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <Select onValueChange={(memberId) => {
                const member = members?.find((m: any) => m._id === memberId);
                if (member) handlePromoteToLead(memberId, member.user_id._id);
              }}>
            ...
            <div className="space-y-3">
              {leadsLoading ? (
                ...
                <p className="text-sm text-muted-foreground">No community leads assigned yet</p>
              ) : (
                leads?.map((lead: any) => (
                  <div key={lead._id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      <Avatar>
                        <AvatarImage src={lead.user_id?.avatar_url} />
                        <AvatarFallback>
                          {lead.user_id?.full_name?.substring(0, 2).toUpperCase() || 'L'}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        <p className="font-medium flex items-center gap-2">
                          {lead.user_id?.full_name || 'Unknown'}
                          <Shield className="w-4 h-4 text-blue-500" />
                        </p>
                        <p className="text-sm text-muted-foreground">{lead.user_id?.email}</p>
                      </div>
                    </div>
                    <Button
                      variant="ghost"
