...
export const ReconciliationTab = () => {
	// Fetch real data from donations table and finance API
		...
		...donations.map((d) => ({
			...
			status:
				d.payment_status === "completed"
					? ("matched" as const)
					: ("pending" as const),
			bankReference: d.payment_reference,
			reconciledAt: d.payment_status === "completed" ? d.updated_at : undefined,
			notes: d.notes,
			source: "api" as const,
		})),
		...expenses.map((e) => ({
			...
			status:
				e.status === "completed" ? ("matched" as const) : ("pending" as const),
			bankReference: e.reference_number,
	...
	const unmatchedCount = reconciliationItems.filter(
					...
					<div className="mb-4">
						...
						<div className="w-full bg-gray-200 rounded-full h-2">
							<div
								...
								style={{ width: `${reconciledPercentage}%` }}></div>
						</div>
					</div>

					<div className="flex justify-between items-center">
						<div className="text-sm text-gray-600">
							Last reconciliation: {new Date().toLocaleDateString()} at{" "}
