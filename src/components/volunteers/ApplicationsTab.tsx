...
export const ApplicationsTab = () => {
	...
	const realApplications = applicationsData?.data || [];

	// Debug logging for data updates
	console.log(
		"📊 Applications data updated at:",
		new Date(dataUpdatedAt || 0).toLocaleTimeString()
	);
	console.log("📋 Real applications count:", realApplications.length);
	console.log("🎯 Status filter:", statusFilter);

	// Mutation hooks for approve/reject
	const approveApplication = useApproveVolunteerApplication();
	const rejectApplication = useRejectVolunteerApplication();
	...
	// Transform real applications to match expected structure
	const transformedApplications = realApplications.map((app) => ({
		...app,
		applicantName: `${app.first_name} ${app.last_name}`,
		name: `${app.first_name} ${app.last_name}`,
		preferredAreas: app.interests || [],
		applicationDate: app.applied_at,
		address: app.address || "Not specified",
		emergencyContact: app.emergency_contact || "Not specified",
		availability: app.availability || "Not specified",
		experience: app.experience || "Not specified",
		references: app.references || [],
		backgroundCheck: app.background_check || "pending",
		interviewScheduled: app.interview_scheduled || false,
		notes: app.notes || "",
	}));

	// Use real applications if available, otherwise fallback to mock data
	const applications =
	...
	const handleApprove = async (applicationId: string) => {
		try {
			...
			console.log("Approve mutation loading:", approveApplication.isPending);

			const result = await approveApplication.mutateAsync({
				id: applicationId,
			...
			await refetch();
			console.log("🔄 Manual refetch completed for approve");
		} catch (error) {
	...
	const handleReject = async (applicationId: string) => {
		try {
			console.log("🔴 Starting reject process for application:", applicationId);
			console.log("🔄 Reject mutation loading:", rejectApplication.isPending);
			...
			console.log("🔄 Mutation completed, cache should be invalidated");

			// Force refetch to ensure UI updates
			await refetch();
			console.log("🔄 Manual refetch completed");

			// If user is viewing pending applications, let them know the application moved
					...
					<div className="space-y-4">
						<div>
							...
							<div className="flex flex-wrap gap-2 mt-2">
								{application.preferredAreas.map(
					...
					{application.status === "approved" && (
						<div className="p-4 bg-green-50 border border-green-200 rounded-lg">
							...
							<p className="text-sm text-green-700 mt-1">
								Approved on{" "}
								{format(new Date(application.approvalDate), "MMMM dd, yyyy")}
							</p>
						</div>
					)}
					...
					{application.status === "rejected" && (
						<div className="p-4 bg-red-50 border border-red-200 rounded-lg">
							...
							<p className="text-sm text-red-700 mt-1">
								Rejected on{" "}
								{format(new Date(application.rejectionDate), "MMMM dd, yyyy")}
							</p>
							<p className="text-sm text-red-700 mt-1">
