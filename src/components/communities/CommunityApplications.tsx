...
export const CommunityApplications = ({
	...
	const { data: applications, isLoading } = useCommunityApplications(
		community.id,
		statusFilter
	);
	...
	const handleReject = async () => {
		...
		try {
			await rejectApplication.mutateAsync({
				communityId: community.id,
				applicationId: selectedApplication.id || selectedApplication._id, // Use id if available, fallback to _id
					...
					<div className="space-y-4">
						<div>
							<p className="text-sm text-muted-foreground mb-4">
								...
								<strong>{selectedApplication?.name}</strong>'s application?
							</p>
						</div>
						<div>
