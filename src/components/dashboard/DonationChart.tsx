...
import { useTransactions } from "@/hooks/use-complete-api";
import {
	Select,
	SelectContent,
	SelectItem,
...
export const DonationChart = () => {
	...
	const generateRealData = (): DonationData[] => {
		if (!transactionsData?.data) return [];

		const transactions = transactionsData.data;
		const donations = transactions.filter((t) => t.type === "income");

		// Group donations by date and payment method
		const dailyData = new Map<
			string,
			{ stripe: number; hundi: number; inTemple: number }
		>();
		...
		donations.forEach((donation) => {
			...
			} else if (donation.payment_method === "cash") {
				existing.hundi += amount;
			} else {
				existing.inTemple += amount;
			}

			dailyData.set(date, existing);
		});

		// Convert to chart data format for last 7 days
		...
		for (let i = 6; i >= 0; i--) {
			...
			data.push({
				date: dateStr,
				stripe: dayData.stripe,
				hundi: dayData.hundi,
				inTemple: dayData.inTemple,
				total: dayData.stripe + dayData.hundi + dayData.inTemple,
			});
		}
	...
	const donationData = generateRealData();
						...
						<div className="flex items-center justify-center h-full">
							<div className="text-center">
								...
								<p className="text-sm text-muted-foreground">
									Add some donations to see the chart
								</p>
							</div>
						</div>
					) : (
